...
class A2D23D(Dataset):
    ...
    def get_data_list(self):
        ...
        if self.phase == "train":
            dataset_folders = public_folders[:-2]
        elif self.phase == "validation":
            dataset_folders = public_folders[-2:-1]
        else:
...
class A2D2Diff(Dataset):
    def __init__(self, phase="train", transform=None, image_size=(224, 224), root_path='.'):
        self.root_path = root_path
        self.phase = phase
        self.transform = transform
        self.image_size = image_size
    ...
    def get_data_list(self):
        ...
        for d in os.listdir(image_root_folder):
            if os.path.isdir(os.path.join(self.root_path, "camera_lidar_semantic", d)):
                image_folders.append(d)
        public_folders = [d for d in label_folders if d in image_folders]
        # print(public_folders)
        dataset_folders = None
        if self.phase == "train":
            dataset_folders = public_folders[1:14] + public_folders[15:]
        elif self.phase == "validation":
            dataset_folders = public_folders[0:1]
        else:
            dataset_folders = public_folders[14:15]
        ...
        for i in range(len(dataset_folders)):
            ...
            labels = {}
            speeds = {}
            for d in os.listdir(os.path.join(self.root_path, "camera_resize_new", folder)):
                folder_images[d[:-4]] = os.path.join(self.root_path, "camera_resize_new", folder, d)
        ...
        for i in range(1, len(images)):
            ...
            if self.mode == "compare":
                if self.source_path == None:
                    image_list.append([os.path.join(self.root_path, "camera_resize_new", folder, images[i-1]),  os.path.join(self.mt_path, images[i])])
                else:
...
def gen_resize_data():
    ...
    for folder in public_folders:
        if not os.path.exists(os.path.join(root_path, "camera_resize_new", folder)):
            os.mkdir(os.path.join(root_path, "camera_resize_new", folder))
...
def check_data(filtered_path, type_="vehcile"):
    ...
    resize_data_path = "E:\\a2d2\\camera_resize_new\\20181108_091945"
